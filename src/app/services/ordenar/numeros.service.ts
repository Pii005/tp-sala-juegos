...
export class NumerosService {
  ...
  obtenernumeros(cantidad: number){
    if (cantidad > this.numeros.length) {
    ...
    // Obtener los números seleccionados a partir de los índices
    const numerosSeleccionados = Array.from(indicesSeleccionados).map(indice => this.numeros[indice]);
